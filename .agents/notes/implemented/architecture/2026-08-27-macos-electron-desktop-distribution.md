...
Signing and notarization are separate durable phases. Electron Builder first creates signed ZIP targets with updater configuration but without implicit notarization. Before any Apple submission, the workflow retains both ZIPs and a hash-bound checkpoint; each submission intent is exclusive and its returned receipt is uploaded separately. Every Apple wait is bounded to five minutes per architecture. A new manual run can restore the original run's artifacts only when repository, workflow, commit, tag, version, hashes, and submission identities agree. Missing receipts fail closed instead of causing an automatic replacement submission.

After both submissions are accepted, the workflow extracts those exact applications, verifies their signatures, staples and validates their tickets, and uses prepackaged targets without re-signing to regenerate installer and updater archives. Architecture-specific outputs remain isolated until the update manifests are merged, retaining each blockmap field and x64 legacy lookup. Pending notarization produces a checkpoint summary, not a draft Release.
...
The launch-path regressions resolve actual filesystem targets, accepting macOS aliases while rejecting missing paths and symlink escapes. Main-entry lifecycle regressions control Electron readiness and backend completion independently: quit prevents late backend creation, windows, and update checks; shutdown-related readiness rejection remains silent, while genuine startup failure still reports its error. These checks cover startup and teardown without model calls or recorded-session changes.
...
Release-state regressions reject foreign runs, changed archives, missing or mismatched receipts, ambiguous submission retries, unknown Apple statuses, and invalid updater manifests. Filesystem-backed tests simulate a pending run followed by accepted responses for the same receipt IDs. Workflow guards require checkpoint retention before submission and both acceptances before every final artifact and draft step; a negative control confirms that removing the acceptance gate fails the test. Actual Apple acceptance and final DMG validation remain owned by macOS CI.
...
**Keep one runner waiting or rebuild after every timeout.** Apple queue time can exceed the runner budget, losing ephemeral signed applications and provoking duplicate submissions. Retained checkpoints separate Apple's queue from runner lifetime. Recovery requires the original artifacts within their 30-day retention window and manual investigation if submission acknowledgment was lost.
