...
`,
  ...
  it.each([
    {
      ...
      platformsPath: 'apps/desktop-runtime/platforms.json',
      runtimeName: '@scope/desktop-runtime',
    },
    {
  ...
  ])('traverses application dependencies from $manifestPath', async ({ manifestPath, platformsPath, runtimeName }) => {
    const root = fixture({
      [manifestPath]: {
        name: runtimeName,
    ...
    expect(result.failures).toEqual([
      `${runtimeName} -> @scope/app -> @scope/plugin -> @scope/required`,
    ])
  })
})
