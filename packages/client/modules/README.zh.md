...
Node 侧保留 `ClientModuleRegistry` 与 `ctx.clientModules` 作为公共 API，同时把实现拆成传输无关的注册表状态与 Web 绑定。状态会协调已启用的 Loader 配置项，解析每个 web `dsh.client` 包的 `exports["./client"]`，对构建后的 bundle 取哈希，携带包专属 `dsh.client.external` 请求，并把动态提供方排在消费者之前；Web 绑定负责生成 `/plugins` 定位符、提供 bundle 及其 sourcemap，并贡献 HTML 启动行。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
