...
describe('client bundle activation', () => {
  ...
  it('serves a packaged desktop client from its runtime tree without Node loader internals', async () => {
    const packageName = '@fixture/desktop-client'
    writeBuiltPackage(packageName, {})
    const runtimeRoot = join(root!, 'DSH.app', 'Contents', 'Resources', 'runtime')
    mkdirSync(runtimeRoot, { recursive: true })
    renameSync(join(root!, 'node_modules'), join(runtimeRoot, 'node_modules'))

    const { service, route } = constructWithRoute([packageName], {
      contextBaseUrl: pathToFileURL(join(root!, 'user-home', 'profiles')).href + '/',
      entryBaseUrl: pathToFileURL(runtimeRoot).href + '/',
    })

    expect(service.clientPath(packageName)).toBe(
      join(runtimeRoot, 'node_modules', '@fixture', 'desktop-client', 'lib', 'client.js'),
    )
    expect(service.graph().entries.map(entry => entry.id)).toEqual([packageName])
    const response = await routeRequest(route, service.graph().batches[0]!.url)
    expect(response.status).toBe(200)
    expect(response.body.toString('utf8')).toContain('module.exports = {}')
  })
