...
The Node half keeps `ClientModuleRegistry` and `ctx.clientModules` as its public API while splitting their implementation into transport-independent registry state and a Web binding. The state reconciles enabled Loader entries, resolves each web `dsh.client` package's `exports["./client"]`, hashes built bundles, carries package-specific `dsh.client.external` requests, and orders dynamic providers before consumers; the Web binding supplies `/plugins` locators, serves each bundle and source map, and contributes the HTML boot rows. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
