...
`desktopCapability` selects per-launch desktop authentication in place of browser sessions. It requires an unpadded base64url capability for 32 random bytes and the exact `x-dsh-desktop-capability` request header. Connection applies it to the frontend index, `/api`, dedicated RPC channels, and sibling routes using `requestRejection`, including Gateway's `/api/remote.mux` upgrade. Missing or incorrect capabilities return 401; browser cookies and query tokens cannot substitute. Desktop mode creates no browser-session credential record or cookie, and `authenticatedUrl` returns the supplied URL unchanged. The desktop process owner supplies the capability outside renderer JavaScript.
...
**Runtime invariant:** No companion is published. Authentication compares request headers against a capability or browser-session secret loaded during activation; it owns no independently mutable identity relation. Stream/reconnect sequencing and rpcId round-trip discipline are exercised directly by behavior specs, and route register/dispose symmetry is audited by the webserver companion.
