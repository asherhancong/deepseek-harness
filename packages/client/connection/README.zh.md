...
`desktopCapability` 选择每次启动独立的桌面认证，替代浏览器会话。它要求 32 个随机字节的无 padding base64url 能力凭据，以及完全匹配的 `x-dsh-desktop-capability` 请求头。Connection 将此校验用于前端 index、`/api`、独立 RPC channel，以及通过 `requestRejection` 认证的其他路由，包括 Gateway 的 `/api/remote.mux` upgrade。凭据缺失或错误返回 401；浏览器 cookie 和 query token 不能替代它。桌面模式不创建浏览器会话凭据记录或 cookie，`authenticatedUrl` 原样返回传入的 URL。桌面进程所有者在 renderer JavaScript 之外提供该能力凭据。
...
**运行时不变式：** 不发布伴生入口。认证会将请求头与能力凭据或激活时加载的浏览器会话密钥比较，不拥有可独立变化的身份关系。流与重连时序、rpcId 往返规则由行为测试直接覆盖，路由注册与释放的对称性由 webserver 伴生入口审计。
