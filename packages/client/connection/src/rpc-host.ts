...
import { hasDesktopCapability, isTrustedApiRequest } from './api-request-trust.ts'
...
export class HostConnectionService extends Service implements HostConnectionHandle {
   ...
   * @param authentication - browser-session owner or per-launch desktop capability.
   */
  constructor(
    ctx: Context,
    private readonly trustedHosts: readonly string[],
    private readonly authentication: BrowserAuth | { readonly desktopCapability: string },
  ) {
  ...
  /** Apply the configured Host/Origin fence, then browser or desktop authentication. */
  requestRejection(request: ConnectionTrustRequest): ConnectionRequestRejection {
    if (!isTrustedApiRequest(request, this.trustedHosts)) return 403
    if ('desktopCapability' in this.authentication) {
      return hasDesktopCapability(request, this.authentication.desktopCapability) ? undefined : 401
    }
    return this.authentication.isAuthenticated(request) ? undefined : 401
  }

  /** Authenticate the index through desktop request checks or browser token/cookie exchange. */
  ...
  /** Return the desktop URL unchanged, or add the browser process's launch token. */
  authenticatedUrl(baseUrl: string): string {
    return 'desktopCapability' in this.authentication
      ? baseUrl
      : this.authentication.authenticatedUrl(baseUrl)
  }
