...
export interface HostConnectionHandle {
  ...
  /**
   * Apply Connection's Host/Origin checks and configured authentication to
   * another Web route.
  ...
  /**
   * Authenticate one frontend index request, owning a browser-token redirect
   * or authentication/trust rejection. Desktop mode requires its capability
   * header and creates no browser cookie.
  ...
  /**
   * Add the fresh process token to a Web URL, or preserve a desktop URL.
   * @param baseUrl - clean canonical browser origin.
   * @returns browser login URL, or the unchanged URL in desktop mode.
   */
