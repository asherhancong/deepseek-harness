...
import {
  API_PATH, RpcId, apply, inject,
  type ClientRequest, type ConnectionConfig, type HostConnectionHandle,
} from '../src/index.ts'
...
describe('connection node half', () => {
  it('fails the load on a malformed desktop capability before registering routes', async () => {
    const ctx = new Context()
    const routes: WebRoute[] = []
    provideBrowserCredentials(ctx)
    ctx.provide('webServer', fakeHttpServer(routes, []) as WebServer)
    await expect(apply(ctx, { desktopCapability: 'guessable' }))
      .rejects.toThrow(/exactly 32 random bytes/)
    expect(routes).toHaveLength(0)
  })

  it('requires desktop capability for API requests and the policy shared by WebSocket routes', async () => {
    const capability = 'A'.repeat(43)
    const { routes, connection, dispose } = await mounted({ desktopCapability: capability })
    let reads = 0
    connection.fetch.register({
      path: '/api/session.export',
      methods: ['GET'],
      fetch: async () => { reads += 1; return new Response('session export') },
    })
    try {
      for (const supplied of [undefined, 'B'.repeat(43)]) {
        ...
        await routes[0]!.handler(request, denied.response)
        expect(denied.state).toMatchObject({ status: 401, body: 'unauthorized' })
      }
      expect(reads).toBe(0)
      const allowed = fakeRequest({
        host: '127.0.0.1:3080',
        [DESKTOP_CAPABILITY_HEADER]: capability,
      }, '/api/session.export')
      expect(connection.requestRejection(allowed)).toBeUndefined()
      const response = fakeResponse()
      await routes[0]!.handler(allowed, response.response)
      expect(response.state).toMatchObject({ status: 200, body: 'session export' })
      expect(reads).toBe(1)
    } finally {
      await dispose()
    }
  ...
  it('authenticates desktop index requests without a browser cookie or launch-token URL', async () => {
    const capability = 'A'.repeat(43)
    const { connection, dispose } = await mounted({ desktopCapability: capability })
    try {
      ...
      for (const path of ['/', '/index.html']) {
        ...
        expect(connection.authorizeIndex(fakeRequest({
          host: '127.0.0.1:3080',
          [DESKTOP_CAPABILITY_HEADER]: capability,
        }, path), allowed.response)).toBe(true)
        expect(allowed.state).toEqual({})
      }
      for (const [headers, status] of [
        [{ host: '127.0.0.1:3080' }, 401],
        [{ host: '127.0.0.1:3080', [DESKTOP_CAPABILITY_HEADER]: 'B'.repeat(43) }, 401],
        [{ host: 'attacker.example', [DESKTOP_CAPABILITY_HEADER]: capability }, 403],
        [{ host: '127.0.0.1:3080', origin: 'http://attacker.example', [DESKTOP_CAPABILITY_HEADER]: capability }, 403],
      ] as const) {
        for (const method of ['GET', 'HEAD']) {
          const denied = fakeResponse()
          const request = fakeRequest(headers, '/?token=browser-token')
          request.method = method
          expect(connection.authorizeIndex(request, denied.response)).toBe(false)
          expect(denied.state.status).toBe(status)
          expect(denied.state.headers?.['cache-control']).toBe('no-store')
          expect(denied.state.headers?.['set-cookie']).toBeUndefined()
          expect(denied.state.body).toBe(method === 'HEAD'
            ? undefined
  ...
  it.each(['browser', 'desktop'] as const)('provides a disposable dedicated RPC channel with %s authentication', async (mode) => {
    const ctx = new Context()
    const routes: WebRoute[] = []
    provideBrowserCredentials(ctx)
    ctx.provide('webServer', fakeHttpServer(routes, []) as WebServer)
    const capability = 'A'.repeat(43)
    const fiber = ctx.plugin({ inject: [...inject], apply }, mode === 'desktop'
    ...
    const denied = fakeResponse()
    await route!.handler(fakePost({ host: '127.0.0.1:3080' }, '/rpc/goals/create', request), denied.response)
    expect(denied.state).toMatchObject({ status: 401, body: 'unauthorized' })
