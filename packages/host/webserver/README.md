...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). The macOS Electron application starts its packaged `dsh web` closure and loads this browser carrier through an owned loopback origin; runtime staging and deferred native IPC remain owned by the [desktop distribution decision](../../../.agents/notes/implemented/architecture/2026-08-27-macos-electron-desktop-distribution.md). This package never prints; the URL line belongs to the shell.
